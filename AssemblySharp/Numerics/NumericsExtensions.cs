...
{
	public static byte[] GetBytes<TNumberType>(this TNumberType number)
		where TNumberType : INumberBase<TNumberType> =>
		typeof(TNumberType).Name switch
		{
			...
			nameof(Decimal) => [.. decimal.GetBits(decimal.CreateChecked(number)).SelectMany(BitConverter.GetBytes)],
			_ => throw new NotSupportedException($"There is no definition for getting bytes from a number of type '{typeof(TNumberType).Name}'"),
		};

	public static int GetBitsSize<TNumberType>(this TNumberType number)
		where TNumberType : INumberBase<TNumberType> =>
		typeof(TNumberType).Name switch
		{
